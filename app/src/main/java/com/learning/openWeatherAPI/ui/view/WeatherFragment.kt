...
class WeatherFragment : Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        weatherViewModel.currentForecastModel.observe(this, Observer {
            binding.apply {
                currentWeatherTv.text = "Сейчас в Саранске:"
                currentTempTv.text =  "Температура:  ${it.main.temp} ºC"
                currentWindTv.text = "Скорость ветра: ${(it.wind.windSpeed * 3.6).roundToInt()} км/ч"
                currentSkyTv.text = "Облачность: ${it.weather.get(0).weatherDesc}"
                currentHumidityTv.text = "Влажность: ${it.main.humidity}%"
            }
